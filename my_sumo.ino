...
#define FULL_SPEED_DURATION_LIMIT     5000  // ms

unsigned long last_ping;
boolean start_to_look;

// Sound Effects
ZumoBuzzer buzzer;
const char sound_effect[] PROGMEM = "O4 T100 V15 L4 MS g12>c12>e12>G6>E12 ML>G2"; // "charge" melody; use V0 to suppress sound effect; v15 for max volume
 
// Timing
unsigned long loop_start_time;
unsigned long last_turn_time;
unsigned long contact_made_time;
#define MIN_DELAY_AFTER_TURN          300  // ms = min delay before detecting contact event
#define MIN_DELAY_BETWEEN_CONTACTS   4000  // ms = min delay between detecting new contact event
...
void loop()
{
	...
	if (start_to_look && ((millis() - last_ping) > 50)) {
		last_ping = millis();
		unsigned int uS = sonar.ping();
		if ((uS / US_ROUNDTRIP_CM) < 40) {
			setForwardSpeed(FullSpeed);
			motors.setSpeeds(getForwardSpeed(), getForwardSpeed());
		}
	}
